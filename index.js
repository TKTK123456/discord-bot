...
async function getPrefixs() {
  let prefixs = fs.readFileSync(`${__dirname}/prefixs.txt`,"utf-8").split("\n");
...
client.on('messageCreate', (message) => {
  const prefix = prefixList.find(p => p.startsWith(message.guild.id)).split(": ");
  if (message.content.toLowerCase().startsWith(`${prefix[1]}setprefix`)) {
    if (message.member.permissions.has("Administrator")) {
      const newPrefix = message.content.split(" ")[1];
      console.log(newPrefix);
      if (newPrefix) {
        prefixList = prefixList.filter(p => p.startsWith(message.guild.id));
        prefixList.set(`${message.guild.id}: ${newPrefix}\n`);
